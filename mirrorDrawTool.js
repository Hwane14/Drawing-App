function MirrorDrawTool() {
	...
	this.unselectTool = function() {
		updatePixels();
		select(".options").html("");
        strokeWeight(1);// Resets the stroke weight so it won't carry over to the other tools
	};
	...
	this.populateOptions = function() {
        ...
        //Direction button HTML creator
		select(".options").html(
			"<br><br><button id='directionButton'>Make Horizontal</button>", true);
        
        
        /* *Side Note* Always create HTML elements first before coding any events otherwise there will be functionality issues(in my case, only 2nd event handler works) */
        
        //Click handler for pen size
		...
		//Click handler for direction button
